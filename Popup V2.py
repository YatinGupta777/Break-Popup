...
time_to_destroy_popup = 2*(animation_distance/60) + stay/60 + 3 # animation_distance and stay needs to be divided by 60 to
																# convert into seconds. +2 for proper functioning
def popup(gif_name,msg):
    ...
    animation = pyglet.image.load_animation(gif_location + gif_name)
    animation = pyglet.image.load_animation(gif_location + gif_name)
    # Animation object that is used on window
    animSprite = pyglet.sprite.Sprite(animation)
    	 
    scale_x = min(popup_width, animSprite.width)/max(popup_width, animSprite.width)
    scale_y = min(popup_height, animSprite.height)/max(popup_height, animSprite.height)
    
    animSprite.scale = max(scale_x, scale_y)
    w = animSprite.width + text_width
    h = animSprite.height
	...
	#animSprite is the gif animation
    animSprite.position = (animSprite.width,0) # Position is relative to window
...
''' Task Scheduling'''
#schedule.every(water_drinking_interval).minutes.do(popup,water_gif_name,water_drinking_message)

popup(water_gif_name,water_drinking_message)
#popup(eye_gif_name,eye_message)
